...
get_data <- function(scenario, vaccine_dose, vaccine_cov){
  data <- read_csv(paste0("modelled_events/impact_10y_mean_events_averted_country_", scenario, ".csv"), col_types = "cccdddd")
...
ranking <- function(scenario, vaccine_dose, vaccine_cov, constraint){
  ...
  # Save output
  filename <- paste0("processed_outputs_", scenario, "/ranking_country_", scenario, "_", vaccine_dose, "_", vaccine_cov, "_", constraint/1e6, "_10y.csv")
...
write_csv(out, "processed_outputs/ranking_summary_all_country_10y.csv")
