...
combine_admin1_rankings <- function(s, vc, vd, con){
  dat <- read_csv(paste0("processed_outputs_", s, "/ranking_admin1_", s, "_", vd, "_", vc, "_", con/1e6, ".csv")) %>% rename("schedule" = "vaccine_type")
  dat$scenario <- s
  dat$vaccine_cov <- vc
  dat$vaccine_dose <- vd
  dat$constraint <- con
...
vc <- c("full_coverage", "reduced_coverage", "reduced_coverage_upper", "reduced_coverage_lower")
vd <- c("4_dose", "3_dose", "both")
...
write_csv(df_country, "processed_outputs/ranking_combined_all_country.csv")
write_csv(df_admin1, "processed_outputs/ranking_combined_all_admin1.csv")
