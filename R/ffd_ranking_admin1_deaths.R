...
get_data <- function(scenario, vaccine_dose, vaccine_cov){
  data <- read_csv(paste0("modelled_events/impact_5y_mean_events_averted_admin1_", scenario, ".csv"), col_types = "iccddddc")
...
ranking <- function(scenario, vaccine_dose, vaccine_cov, constraint){
  ...
  while (total_vacc_doses <= constraint) {
    if (inputs$num_vacc_doses[j] <= (constraint - total_vacc_doses)) {
      ...
      inputs <- dplyr::filter(inputs, DIDE_CODE != priority_list$DIDE_CODE[i])
      i <- i + 1
      j <- 1
    } else {
      if (j >= (nrow(inputs)-1)) {
  ...
  # Save output
  filename <- paste0("processed_outputs_", scenario, "/ranking_admin1_", scenario, "_", vaccine_dose, "_", vaccine_cov, "_", constraint/1e6, "_deaths.csv")
...
vaccine_cov <- c("full_coverage", "reduced_coverage", "reduced_coverage_upper", "reduced_coverage_lower")
constraint <- seq(1e7, 6e7, 1e7)
vars <- crossing(scenario, vaccine_dose, vaccine_cov, constraint)
out <- purrr::pmap_dfr(vars, ranking)

write_csv(out, "processed_outputs/ranking_summary_all_admin1_deaths.csv")
