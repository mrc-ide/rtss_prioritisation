# Prioritise pilots results table
...
x <- read_csv("processed_outputs/GKM_ranking_combined_all_country.csv") %>%
  ...
  summarise(num_countries = length(unique(ISO)), country_list = paste(unique(ISO), collapse = ', '))

y <- read_csv("processed_outputs/GKM_ranking_combined_all_admin1.csv") %>%
  dplyr::filter(vaccine_dose == "4_dose") %>%
  dplyr::select(c(ISO, scenario, vaccine_cov, constraint, level)) %>%
  group_by(scenario, vaccine_cov, constraint, level) %>%
  summarise(num_countries = length(unique(ISO)), country_list = paste(unique(ISO), collapse = ', '))
...
write_csv(table_S5, "results/table_S5.csv")
