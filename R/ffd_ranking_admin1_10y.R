...
get_data <- function(scenario, vaccine_dose, vaccine_cov){
  data <- read_csv(paste0("modelled_events/impact_10y_mean_events_averted_admin1_", scenario, ".csv"), col_types = "iccddddc")
...
ranking <- function(scenario, vaccine_dose, vaccine_cov, constraint){
  ...
  # Save output
  filename <- paste0("processed_outputs_", scenario, "/ranking_admin1_", scenario, "_", vaccine_dose, "_", vaccine_cov, "_", constraint/1e6, "_10y.csv")
...
write_csv(out, "processed_outputs/ranking_summary_all_admin1_10y.csv")
